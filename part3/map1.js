...
const stationMarkers1 = [];  // 存储所有 station circleMarker 引用
    ...
    fetch(`geojson/isochrones_${mins}min/${fileId}_${mins}min.geojson`)
      ...
      .then(data => {
        const layer = L.geoJSON(data, {
          style: {
            color: '#666', weight: 1,
            fillColor: fillColors1[mins], fillOpacity: 0.4
          }
        });
  ...
  fetch(`geojson/merged_isochrones_${mins}min_with_id.geojson`)
    ...
    .then(data => {
      ...
      const layer = L.geoJSON(filtered, {
        style: {
          color: '#666', weight: 1,
          fillColor: fillColors1[mins], fillOpacity: 0.25
        }
      }).addTo(map1);
...
document.getElementById('clear-btn').addEventListener('click', () => {
  clearIsochroneLayers1();
  Object.keys(mergedLayers1).forEach(mins => {
    if (mergedLayers1[mins]) {
      map1.removeLayer(mergedLayers1[mins]);
      mergedLayers1[mins] = null;
    }
  });
